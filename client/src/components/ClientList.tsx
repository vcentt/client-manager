...
const InvoiceList = () => {
  ...
  const getClients = async () => {
    try {
      const response = await axios.get('http://localhost:5177/Client/All-Clients');
  ...
  const handleViewDetails = async (clientId: number) => {
    try {
      const response = await axios.get(`http://localhost:5177/Client/${clientId}`);
  ...
  const handleToDelete = (clientId: number | undefined) => {
    try {
      axios.delete(`http://localhost:5177/Client/${clientId}`);
